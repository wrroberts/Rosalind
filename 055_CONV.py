...
max_multiplicity = max([val for val in spectral_convolution.values()])

# Get the keys corresponding to the maximum multiplicities.
max_x = [item[0] for item in spectral_convolution.items() if item[1] == max_multiplicity]

# Print and save the solution.
print str(max_multiplicity)+'\n'+max_x[0]
with open('output/055_CONV_output.txt','w') as output_file:
    output_file.write(str(max_multiplicity)+'\n'+max_x[0])